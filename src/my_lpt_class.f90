...
contains
    ...
    do k=self%cfg%kmino_,self%cfg%kmaxo_
       do j=self%cfg%jmino_,self%cfg%jmaxo_
          do i=self%cfg%imino_,self%cfg%imaxo_
            ...
            ! In this case, only x- and x+ ghost cells are wall cells

            if (self%cfg%VF(i,j,k).eq.0.0_WP) then
               ! if this cell is wall
               ! just give sign
               self%Wdist(i,j,k)=-sqrt(self%cfg%xL**2+self%cfg%yL**2+self%cfg%zL**2)
            else
               ! if this cell is fluid
               self%Wdist(i,j,k)=+sqrt(self%cfg%xL**2+self%cfg%yL**2+self%cfg%zL**2)
            end if
    ...
    do k=self%cfg%kmino_,self%cfg%kmaxo_
       do j=self%cfg%jmino_,self%cfg%jmaxo_
          do i=self%cfg%imino_+1,self%cfg%imaxo_
             ! if (i) and (i-1) has different sign
  ...
  subroutine advance(this,dt,U,V,W,rho,visc,stress_x,stress_y,stress_z,vortx,vorty,vortz,T,srcU,srcV,srcW,srcE)
    ...
    do i=1,this%np_
       ...
       particleCopy=this%p(i)

       if (this%debugFlag) then 
         write(*,*) "[Debug][Advance] The ID of particle to be advanced: ",  particleCopy%id
         write(*,*) "[Debug][Advance] The Original Position of particle: ",  particleCopy%pos
      end if
       ...
       do while (dt_done.lt.dt)
          ...
          mydt=min(particleCopy%dt,dt-dt_done)
          ! Remember the particle
          pold=particleCopy
          ! Particle moment of inertia per unit mass
          Ip = 0.1_WP*particleCopy%d**2
          ! Advance with Euler prediction
          call this%get_rhs(U=U,V=V,W=W,rho=rho,visc=visc,stress_x=sx,stress_y=sy,stress_z=sz,p=particleCopy,acc=acc,torque=torque,opt_dt=particleCopy%dt)
          particleCopy%pos=pold%pos+0.5_WP*mydt*particleCopy%vel
          particleCopy%vel=pold%vel+0.5_WP*mydt*(acc+this%gravity+particleCopy%Acol)
          particleCopy%angVel=pold%angVel+0.5_WP*mydt*(torque+particleCopy%Tcol)/Ip
          ! Correct with midpoint rule
          call this%get_rhs(U=U,V=V,W=W,rho=rho,visc=visc,stress_x=sx,stress_y=sy,stress_z=sz,p=particleCopy,acc=acc,torque=torque,opt_dt=particleCopy%dt)
          particleCopy%pos=pold%pos+mydt*particleCopy%vel
          particleCopy%vel=pold%vel+mydt*(acc+this%gravity+particleCopy%Acol)
          particleCopy%angVel=pold%angVel+mydt*(torque+particleCopy%Tcol)/Ip
          ! Relocalize
          particleCopy%ind=this%cfg%get_ijk_global(particleCopy%pos,particleCopy%ind)
          ! Send source term back to the mesh
          dmom=mydt*acc*this%rho*Pi/6.0_WP*particleCopy%d**3
          deng=sum(dmom*particleCopy%vel)
          if (this%cfg%nx.gt.1.and.present(srcU)) call this%cfg%set_scalar(Sp=-dmom(1),pos=particleCopy%pos,i0=particleCopy%ind(1),j0=particleCopy%ind(2),k0=particleCopy%ind(3),S=srcU,bc='n')
          if (this%cfg%ny.gt.1.and.present(srcV)) call this%cfg%set_scalar(Sp=-dmom(2),pos=particleCopy%pos,i0=particleCopy%ind(1),j0=particleCopy%ind(2),k0=particleCopy%ind(3),S=srcV,bc='n')
          if (this%cfg%nz.gt.1.and.present(srcW)) call this%cfg%set_scalar(Sp=-dmom(3),pos=particleCopy%pos,i0=particleCopy%ind(1),j0=particleCopy%ind(2),k0=particleCopy%ind(3),S=srcW,bc='n')
          if (present(srcE))                      call this%cfg%set_scalar(Sp=-deng   ,pos=particleCopy%pos,i0=particleCopy%ind(1),j0=particleCopy%ind(2),k0=particleCopy%ind(3),S=srcE,bc='n')
          ! Increment
          dt_done=dt_done+mydt
       end do
       ! Correct the position to take into account periodicity
       if (this%cfg%xper) particleCopy%pos(1)=this%cfg%x(this%cfg%imin)+modulo(particleCopy%pos(1)-this%cfg%x(this%cfg%imin),this%cfg%xL)
       if (this%cfg%yper) particleCopy%pos(2)=this%cfg%y(this%cfg%jmin)+modulo(particleCopy%pos(2)-this%cfg%y(this%cfg%jmin),this%cfg%yL)
       if (this%cfg%zper) particleCopy%pos(3)=this%cfg%z(this%cfg%kmin)+modulo(particleCopy%pos(3)-this%cfg%z(this%cfg%kmin),this%cfg%zL)
       ! Handle particles that have left the domain
       if (particleCopy%pos(1).lt.this%cfg%x(this%cfg%imin).or.particleCopy%pos(1).gt.this%cfg%x(this%cfg%imax+1)) particleCopy%flag=1
       if (particleCopy%pos(2).lt.this%cfg%y(this%cfg%jmin).or.particleCopy%pos(2).gt.this%cfg%y(this%cfg%jmax+1)) particleCopy%flag=1
       if (particleCopy%pos(3).lt.this%cfg%z(this%cfg%kmin).or.particleCopy%pos(3).gt.this%cfg%z(this%cfg%kmax+1)) particleCopy%flag=1
       
       ! Relocalize the particle
       particleCopy%ind=this%cfg%get_ijk_global(particleCopy%pos,particleCopy%ind)

       ! Count number of particles removed
       if (particleCopy%flag.eq.1) this%np_out=this%np_out+1

       ! Copy back to particle
       if (particleCopy%id.ne.-1) this%p(i)=particleCopy

       if (this%debugFlag) then 
         write(*,*) "[Debug][Advance] The Advanced Position of particle: ",  particleCopy%pos
      end if
  ...
  subroutine share(this,nover)
    ...
    do n=1,this%np_
       if (this%p(n)%ind(1).lt.this%cfg%imin_+no) then
          ...
          if (this%debugFlag) then 
            write(*,*) "[Debug][share][X-] The ID of particle to be shared: ",  this%p(n)%id
            write(*,*) "[Debug][share][X-] The X pos and X ind[1] of particle to be shared: ", tosend(nsend)%pos(1), tosend(nsend)%ind(1)
         end if
       end if
    end do
    nrecv=0
    ...
    do n=1,this%np_
       if (this%p(n)%ind(2).lt.this%cfg%jmin_+no) then
          ...
          if (this%debugFlag) then 
            write(*,*) "[Debug][share][Y-] The ID of particle to be shared: ",  this%p(n)%id
            write(*,*) "[Debug][share][Y-] The Y pos and Y ind[1] of particle to be shared: ", tosend(nsend)%pos(2), tosend(nsend)%ind(2)
         end if
       end if
    end do
    nrecv=0
    ...
    do n=1,this%np_
       if (this%p(n)%ind(3).lt.this%cfg%kmin_+no) then
          ...
          if (this%debugFlag) then 
            write(*,*) "[Debug][share][Z-] The ID of particle to be shared: ",  this%p(n)%id
            write(*,*) "[Debug][share][Z-] The Z pos and Z ind[1] of particle to be shared: ", tosend(nsend)%pos(3), tosend(nsend)%ind(3)
         end if
       end if
    end do
    nrecv=0
