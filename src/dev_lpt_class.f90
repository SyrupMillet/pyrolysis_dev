...
contains
   ...
   contains
      function getConvHeatSource(Udif,Vdif,Wdif,visc,charL,rho,gEps,gCp,gk,gT,pT) result(source)
         use mathtools, only: Pi
         implicit none
         real(WP),intent(in) :: Udif,Vdif,Wdif,visc,rho,gCp,gk,gT,pT,charL,gEps
         real(WP) :: absU, Re, Pr, Nu, alpha,A, source
         absU = (Udif**2+Vdif**2+Wdif**2)**0.5
         Re = Rho*absU*charL/visc
         Pr = gCp*visc/gk
         Nu = (7-10*gEps+5*gEps**2)*(1+0.7*Re**0.2*Pr**(1/3))+(1.33-2.4*gEps+1.2*gEps**2)*Re**0.7*Pr**(1/3)
         A = Pi*charL**2.0_WP
         alpha = Nu*gk/charL
         source = alpha*A*(gT-pT)
      end function getConvHEatSource
